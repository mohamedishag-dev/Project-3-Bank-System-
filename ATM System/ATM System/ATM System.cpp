...
enum enMainMenueOptions { eQuickWithdraw = 1, eNormalWithdraw = 2, eDeposit = 3, eCheckBalance = 4, Logout = 5 };
...
void PerfromQuickWithdrawOption(short QuickWithdrawOption)
{
    ...
    short WithdrawOption = GetQuickWithdrawAmount(QuickWithdrawOption);

    if (WithdrawOption > CurrentClient.AccountBalance)
    {
        cout << "\nThe amount exceeds your balance, make another chice";
    ...
    vector <sClient> vClients = LoadCleintsDataFromFile(ClientsFileName);
    DepositBalanceToClientByAccountNumber(CurrentClient.AccountNumber, WithdrawOption * -1, vClients);
    CurrentClient.AccountBalance -= WithdrawOption;
...
void PerfromMainMenueOptions(enMainMenueOptions MainMenueOptions)
{
...
void ShowMainMenue()
{
    ...
    PerfromMainMenueOptions((enMainMenueOptions)ReadMainMenueOption());
}
